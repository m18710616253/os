 ...
 * 
 */
public class RegionPeriodUsageStats implements Program {
    static {
        System.setProperty("log4j.log.app", "regionPeriodUsageStats");
    }
    private static Log log = LogFactory.getLog(RegionPeriodUsageStats.class);
    ...
    /** 补全非活跃线程*/
    private static InactiveStatsFilling inactiveStatsFilling = new InactiveStatsFilling(false);
        
    /** 启动该进程的数据域的名字  */
    private static final String regionName = DataRegion.getRegion().getName();
    ...
    //用于DoStatsTask
    /** 运行统计计算线程的线程池*/
    private static ThreadPoolExecutor es = new ThreadPoolExecutor(10, 100, 1000, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(1000));

    public void exec(String[] args) throws Exception {
        DSyncService dsync = new DSyncService(GlobalHHZConfig.getQuorumServers(), GlobalHHZConfig.getSessionTimeout());
        try {
            DLock lock = dsync.createLock(OosZKNode.getPeriodUsageStatsLock(regionName), null);
            lock.lock();
            LogUtils.startSuccess();
            
            try {
                // 统计带宽95峰值 分各个region
                new PeakBandWidthStats(false).start();       
                ...
                List<UsageMetaType> typeToStatsList = Lists.newArrayList(UsageMetaType.CURRENT_OWNER, UsageMetaType.CURRENT_BUCKET, UsageMetaType.CURRENT_AVAIL_BW);
                ...
                for (;;) {
                    ...
                    //减少重启次数，使用配置文件清空缓存
                    if (PeriodUsageStatsConfig.regionClearCache.contains(regionName)) {
                        cachedUsageMeta.clear();
                        log.info("clear day meta cache region is : " + regionName);
                    }
                    // 遍历表中的UsageMetaType.CURRENT_OWNER类型，统计到UsageMetaType.MINUTES_OWNER类型，每5分钟一条记录。
                    startTime = lastStatsTime; 
                    // 取（当前时间-usageStatsTimeAgo），以及（startTime + 时间间隔）的最小值，防止每次scan范围太大
                    endTime = Math.min((nowTime - usageStatsTimeAgo), (startTime + interval));
                    startUp = false;
                    try {                                               
                        log.info("start Stats per "+ interval +". from " + startTime + " to "+ endTime);
                        
                        //current_owner 、 current_bucket 以及CURRENT_AVAIL_BW
                        for (UsageMetaType type : typeToStatsList) {
                            doOwnersCurrentUsageStats(type);
                        }
    ...
    /**
     * 计算owenr级别和bucket级别以及可用带宽的current数据
     * @param type
     * @throws Exception
     */
    private void doOwnersCurrentUsageStats(UsageMetaType type) throws Exception {
        byte[] nextKey = null;
        byte[] oldNextKey = null;
        for (;;) {
            long t1 = System.currentTimeMillis();
            LinkedHashMap<String, TreeMap<Long, MinutesUsageMeta>> minutesUsages = client
                    .minutesUsageRegionCurrent5Mins(startTime,
                            endTime, regionName, -1, null,
                            type,
                            nextKey, 5000);                                                   
            long t2 = System.currentTimeMillis() - t1;
            
            log.info("UsageMetaType=" + type + ", minutesUsageRegionCurrent5Mins time=" + t2+" minutesUsages.size:"+minutesUsages.size());
            
            //兼容旧版本api写入的current数据，加和到新版本上。下一个版本可以删除
            oldNextKey = bulidMinutesUsages(minutesUsages, type, oldNextKey, startTime, endTime);
            
            if (minutesUsages.isEmpty()) {
                log.info("UsageMetaType=" + type + ", statsTime=" + Misc.formatyyyymmddhhmm(new Date(endTime)));
                break;
            }
            
            long t3 = System.currentTimeMillis();
            if (type == UsageMetaType.CURRENT_AVAIL_BW) {
                nextKey = doAvailableBWStats(minutesUsages, type);
            } else {
                nextKey = doStats(minutesUsages, type);
            }
            long t4 = System.currentTimeMillis() - t3;
            log.info("UsageMetaType=" + type + ", doStats time=" + t4+" minutesUsages.size:"+minutesUsages.size());
        }
    }
     ...
     * @throws Exception
     */
    private byte[] bulidMinutesUsages(LinkedHashMap<String, TreeMap<Long, MinutesUsageMeta>> minutesUsages,
            UsageMetaType usageMetaType, byte[] oldNextKey, long startTime, long endTime) throws Exception {
        
        LinkedHashMap<String, TreeMap<Long, MinutesUsageMeta>> oldMinutesUsages = client
                .minutesUsageCurrentAll5Mins(startTime,
                        endTime, regionName, -1, null,
                        usageMetaType,
                        oldNextKey, 5000);
        log.info("UsageMetaType=" + usageMetaType + ", minutesUsageCurrentAll5Mins minutesUsages.size: "+ oldMinutesUsages.size());

        if (oldMinutesUsages.isEmpty()) {
            return oldNextKey;
        }
        
        for(Entry<String, TreeMap<Long, MinutesUsageMeta>> entry : oldMinutesUsages.entrySet()) {            
            String key = changeKey(entry.getKey(), usageMetaType);
            Pair<byte[], Long> toDelete = new Pair<byte[], Long>(Bytes.toBytes(entry.getKey()), -1l);
            if (minutesUsages.containsKey(key)) {
                TreeMap<Long, MinutesUsageMeta> oldTreeMap = entry.getValue();
                TreeMap<Long, MinutesUsageMeta> treeMap = minutesUsages.get(key);
                for(Entry<Long, MinutesUsageMeta> oldEntry : oldTreeMap.entrySet()) {
                    if (treeMap.containsKey(oldEntry.getKey())) {
                        treeMap.get(oldEntry.getKey()).sumAllExcpetPeak(oldEntry.getValue());;
                    } else {
                        treeMap.put(oldEntry.getKey(), oldEntry.getValue());
                    }
                }
            } else {
                minutesUsages.put(key, entry.getValue());
            }
            //删除旧版本的current数据
            toDelete.second(entry.getValue().lastEntry().getValue().timeVersion);
            long ts = MinutesUsageMeta.buildTimeVersion(MinutesUsageMeta.parseTimeVersion(toDelete.second())) + 999999;
            toDelete.second(ts);
            toDeleteList.add(toDelete);
            oldNextKey = Bytes.toBytes(entry.getKey() + Character.MIN_VALUE);
        }
        return oldNextKey;
    }
    
    private String changeKey(String row, UsageMetaType usageMetaType) {
        String[] rows = row.split("\\|");
        int len = rows.length;
        String key = null;
        switch (usageMetaType) {
        case CURRENT_OWNER:
            if (len == 4) {
                key = rows[1] + "|" + rows[0] + "|" + rows[2] + "|" + rows[3];
            } else if(len == 5) {
                key = rows[1] + "|" + rows[0] + "|" + rows[2] + "|" + rows[3]+ "|" + rows[4];
            }
            break;
        case CURRENT_BUCKET:
            if (len == 5) {
                ...
                key = rows[1] + "|" + rows[0] + "|" + rows[2] + "|" + rows[3] + "|" + rows[4] + "|" + rows[5];
            }
            break;
        case CURRENT_AVAIL_BW:            
            key = rows[1] + "|" + rows[0] + "|" + rows[2];
            break;
        default:
            break;
        }
    ...
    private byte[] doStats(LinkedHashMap<String, TreeMap<Long, MinutesUsageMeta>> minutesUsages, UsageMetaType type) throws Exception {
        ...
        for (Entry<String, TreeMap<Long, MinutesUsageMeta>> entry : minutesUsages.entrySet()) {
            if (lastKey == null) {
                ...
                lastKey = entry.getKey().compareTo(lastKey) > 0 ? entry.getKey() : lastKey;//取字典序最大
            }
    ...
    class DoStatsTask implements Callable<Boolean> {
        ...
        public Boolean call() throws Exception {
            for (Entry<String, TreeMap<Long, MinutesUsageMeta>> entry : minutesUsages.entrySet()) {
                ...
                // 行key，格式为regionName|CURRENT_XXX|userId|(bucketName|)day
                // 低频和标准混合 regionName|CURRENT_XXX|userId|(bucketName|)(storageType|)day
                ...
                for (Entry<Long, MinutesUsageMeta> usageKV : v.entrySet()) {
                    ...
                    if (hourMeta != null && hourMeta.fiveMinutesInMills > floorHourStartTime) {
                        ...
                        if (!client.minutesUsageSelect(hourMeta)) {
                            ...
                            hourMeta.initStatsExceptConnection();
    ...
    private byte[] doAvailableBWStats(LinkedHashMap<String, TreeMap<Long, MinutesUsageMeta>> minutesUsages, UsageMetaType type) throws Exception {
        ...
        for(Entry<String, TreeMap<Long, MinutesUsageMeta>> entry : minutesUsages.entrySet()) {
            ...
            if (lastKey == null) {
                ...
                lastKey = k.compareTo(lastKey) > 0 ? k : lastKey;
            }            
            //某个五分钟的MinutesUsageMeta，按时间从小到大排序
            TreeMap<Long, MinutesUsageMeta> v = entry.getValue();
            //已统计的最新的UsageMetaType.DAY_OWNER、UsageMetaType.DAY_BUCKET统计
    ...
    private long bulidRegionLastSizeMap(Map<String, SizeStats> regionLatestSize, String storageType, String lastFillToDate) throws Exception {
        ...
        rowkeyList.add(regionName + "|" + storageType);
    ...
    public static void main(String[] args) throws Exception {
        new RegionPeriodUsageStats().exec(args);
    }
